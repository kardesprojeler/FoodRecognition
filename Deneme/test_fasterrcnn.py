...
model_classifier_only = tf.keras.models.Model([feature_map_input, roi_input], classifier)
...
for idx, img_name in enumerate(sorted(os.listdir(r"C:\Users\BULUT\Desktop\test_images"))):
    if not img_name.lower().endswith(('.bmp', '.jpeg', '.jpg', '.png', '.tif', '.tiff')):
        continue
    print(img_name)
    st = time.time()
    filepath = os.path.join(r"C:\Users\BULUT\Desktop\test_images", img_name)
    ...
    for jk in range(R.shape[0] // C.num_rois + 1):
        ...
        for ii in range(P_cls.shape[1]):
            ...
            cls_name = classes[np.argmax(P_cls[0, ii, :])]
    ...
    for key in bboxes:
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            (real_x1, real_y1, real_x2, real_y2) = get_real_coordinates(1, x1, y1, x2, y2)
